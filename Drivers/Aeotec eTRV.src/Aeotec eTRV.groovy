/**
		...
		metadata {
			...
			//options for configLoggingLevelIDE
			def configLoggingOptions = [:]
				configLoggingOptions << ["0" : "None"] // 0x00
				configLoggingOptions << ["1" : "Error"] // 0x01
				configLoggingOptions << ["2" : "Warning"] // 0x02
				configLoggingOptions << ["3" : "Info"] // 0x03
				configLoggingOptions << ["4" : "Debug"] // 0x04
				configLoggingOptions << ["5" : "Trace"] // 0x05
			...
			preferences {
                ...
                input name: "configLoggingLevelIDE", type: "enum", defaultValue: "3", title: "IDE Live Logging Level:\nMessages with this level and higher will be logged to the IDE.", description: "Default: Info", options: configLoggingOptions, required: false, displayDuringSetup: true
			}
		  ...
		  //log.debug "Parsing '${description}'"
			...
			if (description.startsWith("Err 106")) {
				...
				if (cmd) {
					...
					logger("Non-parsed event: ${description}", "debug")
				}
			}
			return result
		}
	...
	def zwaveEvent(hubitat.zwave.commands.batteryv1.BatteryReport cmd) {
		...
		logger("Report Received : $cmd", "info")
		createEvent(map)
	}
		...
		def zwaveEvent(hubitat.zwave.commands.protectionv1.ProtectionReport cmd) {
			...
			logger("Protection State - ${eventValue}", "info")
			return event
		}
		...
		def zwaveEvent(hubitat.zwave.commands.switchmultilevelv3.SwitchMultilevelReport cmd){
			...
			logger("Report Received : Valve open ${cmd.value}%", "info")
			return event
		}
	...
	def zwaveEvent(hubitat.zwave.commands.thermostatsetpointv2.ThermostatSetpointReport cmd) { //	Parsed ThermostatSetpointReport(precision: 2, reserved01: 0, scale: 0, scaledValue: 21.00, setpointType: 1, size: 2, value: [8, 52])
		...
		def zwaveEvent(hubitat.zwave.commands.basicv1.BasicReport cmd){
			...
			logger("Report Received : ${cmd}, ${state.thermostatMode}, ${state.thermostatOperatingState}", "info")
			return event
		}
		...
		def zwaveEvent(hubitat.zwave.commands.thermostatmodev2.ThermostatModeSupportedReport cmd) {
			...
			logger("Report Received : $cmd, Thermostat supported modes : $supportedModes", "info")
		}
		...
		def zwaveEvent(hubitat.zwave.commands.notificationv3.NotificationReport cmd) {
			...
			logger("Notification Report : $cmd", "info")
		}
		...
		private processPowerMgtNot(cmd) {
			...
			logger("Power management event: Warning! $descriptionText", "info")
		}
		...
		private processSystemNot(cmd) {
			...
			logger("System event: $descriptionText", "info")
		}
		...
		def zwaveEvent(hubitat.zwave.Command cmd) {
			...
			logger("mics zwave.Command - ${device.displayName} - $cmd", "warn")
			sendEvent(map)
		}
	...
	private processParam1(cmd) {
		...
		logger("LCDinvert: ${setValue}", "info");
	...
	private processParam2(cmd) {
		...
		logger("LCDtimeout: ${setValue}", "info")
	...
	private processParam3(cmd) {
		...
		logger("backlight: ${setValue}", "info")
	...
	private processParam4(cmd) {
		...
		logger("battery notification: ${setValue}", "info")
	...
	private processParam5(cmd) {
		...
		logger("Temp report: ${setValue}", "info")
	...
	private processParam6(cmd) {
		...
		logger("Valve report: ${setValue}", "info")
	...
	private processParam7(cmd) {
		...
		logger("Window open detection: ${setValue}", "info")
	...
	private processParam8(cmd) {
		...
		logger("Temp offset: ${setValue}", "info")
		...
		def setHeatingSetpoint(Double degrees) { //Double added
			...
			logger("nextHeatingSetpoint set to ${degrees}", "debug")
	...
	//			cmds << zwave.thermostatSetpointV2.thermostatSetpointGet(setpointType: 11)
			...
			logger("Setting Temp to ${degrees},  $cmds", "trace")
			secureSequence(cmds)
		}
		...
		*/
		...
		//SetValve during DVC
			def setLevel(nextLevel) {
				...
				logger("Executing 'setLevel' : $cmds", "trace")
				secureSequence(cmds)
		}
		...
		def configure() {
            ...
            if (ExternalTemp == 0) {
                ...
                logger("Using External Temperatures", "info")
			...
			logger("Configuration sent", "debug")
			secureSequence(cmds)            
 			}
		...
		sendEvent(name: "checkInterval", value: 2 * 15 * 60 + 2 * 60, displayed: false, data: [protocol: "zwave", hubHardwareId: device.hub.hardwareID])
			if (!state.updatedLastRanAt || new Date().time >= state.updatedLastRanAt + 2000) {
				...
				unschedule(refresh)
				unschedule(poll)
				logger("Configuring settings", "trace")
				runIn (05, configure)
				sendEvent(name: "configure", value: "configdue", displayed: false)
				switch(refreshRate) {
				case "1":
					runEvery1Minute(poll)
					logger("Refresh Scheduled for every minute", "info")
					break
				case "15":
					runEvery15Minutes(poll)
					logger("Refresh Scheduled for every 15 minutes", "info")
					break
				case "10":
					runEvery10Minutes(poll)
					logger("Refresh Scheduled for every 10 minutes", "info")
					break
				case "5":
					runEvery5Minutes(poll)
					logger("Refresh Scheduled for every 5 minutes", "info")
					break
				case "0":
					logger("Refresh off", "info")
                }
		 
			}
			else {
				logger("update ran within the last 2 seconds", "warn")
			}
		}
		...
		def setDeviceLimits() { // for google and amazon compatability
			...
			logger("setDeviceLimits - device max/min set", "trace")
		}	
...
def resetBatteryReplacedDate(paired) {
	...
	logger("Setting Battery Last Replaced to current date${newlyPaired}", "debug")
}
