...
*/
def configure() {
	logDebug "Qubino Flush Shutter: configure()"
	state.isMcDevice = false
	logDebug state.isMcDevice
	def assocCmds = []
...
*/
def stop() {
	logDebug "Qubino Flush Shutter: stop()"
	def cmds = []
...
*/
def stopSlats(){
	logDebug "Qubino Flush Shutter: stopSlats()"
	def cmds = []
...
*/
def open() {
	logDebug "Qubino Flush Shutter: open()"
...
*/
def close() {
	logDebug "Qubino Flush Shutter: close()"
...
*/
def openSlats(){
	logDebug "Qubino Flush Shutter: openSlats()"
...
*/
def closeSlats(){
	logDebug "Qubino Flush Shutter: closeSlats()"
...
def setSlatLevel(level) {
	logDebug "Qubino Flush Shutter: setSlatLevel()"
	logDebug level
...
*/
def calibrate() {
	logDebug "Qubino Flush Shutter: calibrate()"
...
*/
def setLevel(level) {
	logDebug "Qubino Flush Shutter: setLevel()"
	if(level > 99) level = 99
...
def refreshPowerConsumption() {
	logDebug "Qubino Flush Shutter: refreshPowerConsumption()"
	delayBetween([
...
*/
def resetPower() {
	logDebug "Qubino Flush Shutter: resetPower()"
...
def setAssociation() {
	logDebug "Qubino Flush Shutter: setAssociation()"
	def assocSet = []
...
def setConfiguration() {
	logDebug "Qubino Flush Shutter: setConfiguration()"
	def configSequence = []
...
def parse(String description) {
	logDebug "Qubino Flush Shutter: Parsing '${description}'"
	def result = null
...
def zwaveEvent(hubitat.zwave.commands.sensormultilevelv5.SensorMultilevelReport cmd){
	logDebug "Qubino Flush Shutter: SensorMultilevelReport handler fired"
	def resultEvents = []
...
def zwaveEvent(hubitat.zwave.commands.switchmultilevelv3.SwitchMultilevelReport cmd){
	logDebug "Qubino Flush Shutter: firing switch multilevel event"
	def result = []
	result << createEvent(name:"windowShade", value: cmd.value ? "open" : "closed", isStateChange: true)
	if(cmd.value > 99){
		result << createEvent(name:"position", value: cmd.value, unit:"%", descriptionText:"${device.displayName} is uncalibrated! Please press calibrate!", isStateChange: true)
	}else{
		result << createEvent(name:"position", value: cmd.value, unit:"%", descriptionText:"${device.displayName} moved to ${cmd.value==99 ? 100 : cmd.value}%", isStateChange: true)
	}
	return result
}
/**
...
def zwaveEvent(hubitat.zwave.commands.switchmultilevelv3.SwitchMultilevelReport cmd, hubitat.zwave.commands.multichannelv3.MultiChannelCmdEncap command){
	logDebug "Qubino Flush Shutter: firing MC switch multilevel event"
	def result = []
	switch(command.sourceEndPoint){
		case 1:
			result << createEvent(name:"windowShade", value: cmd.value ? "open" : "closed", isStateChange: true)
			if(cmd.value > 99){
				result << createEvent(name:"Position", value: cmd.value, unit:"%", descriptionText:"${device.displayName} is uncalibrated! Please press calibrate!")
			}else{
				result << createEvent(name:"Position", value: cmd.value, unit:"%", descriptionText:"${device.displayName} moved to ${cmd.value==99 ? 100 : cmd.value}%", isStateChange: true)
			}
		break;
		case 2:
			logDebug "Received command from EP2"
			logDebug cmd
...
def zwaveEvent(hubitat.zwave.commands.meterv3.MeterReport cmd, hubitat.zwave.commands.multichannelv3.MultiChannelCmdEncap command){
	logDebug "Qubino Flush Shutter: firing MC Encap meter report event"
	def result = []
	switch(cmd.scale){
		case 0:
...
def zwaveEvent(hubitat.zwave.commands.meterv3.MeterReport cmd) {
	logDebug "Qubino Flush Shutter: firing meter report event"
	def result = []
	switch(cmd.scale){
		case 0:
			result << createEvent(name:"kwhConsumption", value: cmd.scaledMeterValue, unit:"kWh", descriptionText:"${device.displayName} consumed ${cmd.scaledMeterValue} kWh", isStateChange: true)
...
def zwaveEvent(hubitat.zwave.commands.configurationv2.ConfigurationReport cmd){
	logDebug "Qubino Flush Shutter: firing configuration report event"
	logDebug cmd.configurationValue
}
/**
...
def zwaveEvent(hubitat.zwave.commands.basicv1.BasicReport cmd){
	logDebug "Qubino Flush Shutter: firing basic report event"
	def result = []
	result << createEvent(name:"windowShade", value: cmd.value ? "open" : "closed", isStateChange: true)
	if(cmd.value > 99){
		result << createEvent(name:"position", value: cmd.value, unit:"%", descriptionText:"${device.displayName} is uncalibrated! Please press calibrate!", isStateChange: true)
...
def zwaveEvent(hubitat.zwave.commands.multichannelv3.MultiChannelEndPointReport cmd){
	logDebug "Qubino Flush Shutter: firing MultiChannelEndPointReport"
	if(cmd.endPoints > 0){
...
def zwaveEvent(hubitat.zwave.commands.multichannelv3.MultiChannelCapabilityReport cmd){
	logDebug "Qubino Flush Shutter: firing MultiChannelCapabilityReport"
	state.isMcDevice = true
	def cmds = []
	cmds << response(zwave.associationV1.associationRemove(groupingIdentifier:1).format())
	//cmds << "delay 500"
...
def zwaveEvent(hubitat.zwave.commands.multichannelv3.MultiChannelCmdEncap cmd){
	logDebug "Qubino Flush Shutter: firing MC Encapsulation event"
